...
Still untested: `RestaurantEnrichmentService`, `LiveSearchService`, the remaining live-search source `OverpassService` normalize/cache-key logic.
...
## Next
Pick the next uncovered service. `RestaurantEnrichmentService` is the largest gap but requires stubbing/mocking many collaborators — consider testing small behaviors indirectly. `LiveSearchService` is partially covered via `LiveSearchScoringTest` but its search() orchestration is untested. The last free-source `OverpassService` normalizeRaw/cacheKeyFor path matches the pattern just added for BizData/Socrata/SerpApi.
