...
## State
Stats row lives in hero (fade-in, staggered count-up, reduced-motion aware, a11y list). useCountUp now skips redundant re-animation when target unchanged after settling; still re-animates on real changes. 1099 vitest green.

## Log
- ITER-7: useCountUp skips re-animation when target unchanged post-settle (settledTarget guard); 2 new specs (no re-run on same target, re-runs on change). 1099 vitest green.
