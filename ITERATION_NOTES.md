...
## State
- Fixed BackfillRestaurantWebsites.php (1 error), RestaurantEnrichmentService.php (1 error), RestaurantValidationService.php (1 error): added `(string)` cast on `preg_replace` return before passing to `substr`/`strlen`. All three were `preg_replace` returning `string|null` passed directly to string functions. Baseline: 301 → 283 lines.
- Next: 47 errors remain — 21 `property.nonObject` (mostly `$request->user()` nullable on auth-optional routes), 17 `method.nonObject`, 8 `offsetAccess.notFound`, 1 `argument.type`. The property.nonObject errors are the biggest batch; many are `$request->user()` in Inertia controllers that require auth middleware but PHPStan can't see it.
