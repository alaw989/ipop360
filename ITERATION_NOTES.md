...
Still uncovered: `RestaurantEnrichmentService` full `enrichByCuisine` orchestration (`fetchAndNormalizeAllSources`, `Http::pool`, the persisted scoring batch-update CASE WHEN block), `LiveSearchService.search()` orchestration (LiveSearchScoringTest covers scoring only).
