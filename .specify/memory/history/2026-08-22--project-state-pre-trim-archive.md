# Archive: project-state.md pre-trim (as of 2026-08-22)

> `project-state.md` had grown to 564 lines — mostly a spec-by-spec narrative
> (specs 001–103) that duplicated `history.md` and the per-spec files in
> `history/`. It was trimmed on 2026-08-22 to restore its purpose as a living
> snapshot (per its own header: "Detailed per-spec history lives in
> `history/`"). This file preserves the removed sections verbatim for
> reference. Nothing here is newer information than `history.md` — if in
> doubt, trust `history.md` / `history/*.md` over this archive.

---

## From "Latest (2026-08-11, later)" — historical detail (binding rules were kept in project-state.md; this is the rest)
...
**Current floor (STALE as of the 2026-08-15 update above):** 655 PHPUnit + 968 vitest tests; PHPStan level 8
over `app/ + tests/` with a **zero baseline**; pint clean; **CI enforces PHPUnit + vitest coverage thresholds,
...
**Next (STALE):** see `.specify/memory/backlog.md` (next goal at the time = **admin dashboard basic counts**).

---

## Current state (2026-06-26) — superseded, specs 001–103 now all shipped
...
## What's next (queued specs — as of 2026-06-27, ALL NOW SHIPPED — see history.md)
